...
+ Fixed issue on not accepting PAD orders caused by a divide by zero error.
...
+ After complete FastCheckout transaction no order confirmation page was shown
...
+ Multi-stores in WooCommerce are now supported.
...
+ Fixed a bug causing the postcode not to be added to the order when using Fast Checkout.
...
+ Added DB Table to check if order is already created and if so go to normally updating process when using Fast Checkout
...
+ Auto split house number from address if needed
...
## Fixes ##
+ Fixed bug that caused no order data to show on thank you page
